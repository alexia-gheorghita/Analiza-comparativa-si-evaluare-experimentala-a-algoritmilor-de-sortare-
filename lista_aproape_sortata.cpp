...
//GENERARE VECTOR APROAPE SORTAT (95% corect)
void generateAlmostSortedVector(int v[], int n) {
    // 1. Incarcam vectorul sortat
    for (int i = 0; i < n; i++) {
        v[i] = i + 1;
    }

    // 2. Facem cateva swap-uri aleatorii (aprox 5%)
...
int main() {
    ...
    cout << "Rulare experiment: VECTOR APROAPE SORTAT (5% dezordine)" << endl;
    cout << "Testare N = " << n << " pe " << num_tests << " repetari." << endl << endl;

    for (int t = 0; t < num_tests; t++) {
        generateAlmostSortedVector(original, n);
