...
void quickSort(int v[], int low, int high) {
    if (low >= high) return;
    // Chiar daca e sortat, folosim pivot aleatoriu pentru a evita O(n^2) pe QuickSort
...
//GENERARE VECTOR SORTAT
void generateSortedVector(int v[], int n) {
    for (int i = 0; i < n; i++) {
        v[i] = i + 1;
    }
}
...
int main() {
    ...
    cout << "Rulare experiment: VECTOR DEJA SORTAT (" << num_tests << " repetari, N = " << n << ")" << endl;
    cout << "Analiza comportamentului adaptiv..." << endl << endl;

    for (int t = 0; t < num_tests; t++) {
        // Generam vectorul sortat o singura data per test
        generateSortedVector(original, n);

        auto run_test = [&](int alg_idx, auto sort_func) {
            for (int i = 0; i < n; i++) v[i] = original[i]; 
            max_aux_mem = 0; current_aux_mem = 0; 
        ...
        if ((t + 1) % 1000 == 0) cout << "Progres: " << t + 1 << "/" << num_tests << " teste finalizate." << endl;
    }
