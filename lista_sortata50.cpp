...
//GENERARE VECTOR SEMI-SORTAT (50%)
void generateHalfSortedVector(int v[], int n) {
    // Prima jumatate sortata crescator
    for (int i = 0; i < n / 2; i++) {
        v[i] = i + 1;
    }
    // A doua jumatate aleatorie
    for (int i = n / 2; i < n; i++) {
        v[i] = rand() % 10000;
    }
}
...
int main() {
    ...
    cout << "Rulare experiment: VECTOR SEMI-SORTAT (50% crescator, 50% random)" << endl;
    cout << "Testare N = " << n << " pe " << num_tests << " esantioane." << endl << endl;

    for (int t = 0; t < num_tests; t++) {
        generateHalfSortedVector(original, n);
